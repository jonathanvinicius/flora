...
import { BodyParameter, HeaderParameter, QueryParameter } from './dtos';
/**
...
export abstract class BaseService {
  ...
  constructor(
    ...
    this.config = config;
    this._useAgent = ssl;
    this._showLog = process.env.SHOW_AXIOS_LOGS === 'true';
   ...
   * Request HTTP
   */
  async requestAxios<T>(config: AxiosRequestConfig): Promise<AxiosResponse<T>> {
   ...
   * Request GET
   */
  async get<T>(params: QueryParameter): Promise<AxiosResponse<T>> {
    ...
    try {
      ...
      if (params.mapper) {
        res.data = params.mapper.fromJson(res.data);
      }
      return res;
    } catch (err) {
      throw this._handleErrorRequest(err, 'get');
    }
  }
   ...
   * Request POST
   */
  async post<T>(params: BodyParameter): Promise<AxiosResponse<T>> {
    this._handleSsl(params.config);
    const url = params.requestUrl(this.config.url);
    this.logRequest(params);
    try {
   ...
   * Request PUT
   */
  async put<T>(params: BodyParameter): Promise<AxiosResponse<T>> {
    this._handleSsl(params.config);
    const url = params.requestUrl(this.config.url);
    this.logRequest(params);
    try {
   ...
   * Request PATCH
   */
  async patch<T>(params: BodyParameter): Promise<AxiosResponse<T>> {
    this._handleSsl(params.config);
    const url = params.requestUrl(this.config.url);
    this.logRequest(params);
    try {
   ...
   * Request DELETE
   */
  async delete<T>(params: HeaderParameter): Promise<AxiosResponse<T>> {
    this._handleSsl(params.config);
    const url = params.requestUrl(this.config.url);
    this.logRequest(params);
    try {
